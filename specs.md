...
* **Run Detail Page:** The run detail view exposes the supervisor timeline, selected document context, vertically stacked collapsible fundamental, technical, and news agent outputs, and the final supervisor outcome. The first backend-connected slice polls persisted run state rather than streaming tokens live. The elapsed timer shown in the header must count upward only while the workflow is actively running; once the run reaches `completed` or `failed`, it must freeze to the persisted run duration computed from the stored timestamps.
