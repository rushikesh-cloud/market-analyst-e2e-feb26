...
* **Observability:** Opik tracing for LangChain agent executions and direct Azure chat-model analysis calls when `OPIK_*` settings are configured. Top-level agent and sub-agent execution functions also carry `@opik.track(...)` decorators so orchestration spans appear even around non-LangChain control flow.
...
* **Output:** Fundamental Rating (1-100, where 100 is most positive) + structured analyst payload. The worker response must include compact business-summary fields for direct UI rendering when the report supports them, such as stance, top positives, top risks, watch items, revenue or revenue trend, growth signals, cash-flow view, leverage view, profitability view, and valuation/business-quality commentary. Numeric business fields may be null when the retrieved report context does not support a reliable extraction.
...
* **Output:** Technical Rating (1-100, where 100 is most positive) + structured chart-summary payload. The worker response must expose compact fields for direct UI rendering, including stance, trend state, momentum state, setup, support/resistance levels, risk flags, watch items, and chart-derived metrics such as latest close, RSI, MACD state, and distance versus key moving averages when available from the generated artifact.
...
* **Output:** News Rating (1-100, where 100 is most positive) + structured sentiment payload. The worker response must expose stance, sentiment balance, compact positive/negative/watch buckets, sector tailwinds/headwinds, and source links so the frontend can render minimal decision-oriented panels instead of long narrative text.
...
* **Output:** Final Future-Perspective Rating (1-100, where 100 is most positive) with component rating rationale plus a structured top-level summary for the run-detail UI. The supervisor payload must include stance, confidence label, decision summary, top reasons for, top reasons against, watch items, and normalized component-contribution metadata.


5. **Supervisor Chat Agent:**
* **Role:** Answers follow-up questions after or alongside a supervisor run.
* **Logic:** Runs as a second supervisor layer with the fundamental, technical, and news worker agents attached as tools. It routes user questions to the right worker tool when the question needs fresh fundamental, technical, or news evidence, and uses the attached supervisor snapshot for questions about the existing overall rating. The chat entry path must enforce input guardrails before the LLM runs: only market-related questions within the current product scope are allowed, limited to fundamentals, technicals, news, sector context, and the existing supervisor view for the selected stock. Out-of-scope prompts must be refused with a short scope reminder instead of being answered. Prompt-injection and jailbreak attempts must also be blocked at the LangChain middleware layer before normal agent execution continues. When Azure Prompt Shield middleware is configured and available, it should be attached as an additional prompt-injection screen ahead of the supervisor chat agent.
...
* **Output:** Conversational answer grounded in the supervisor snapshot and any worker-tool results used during the turn. When a user asks for a future-looking recommendation, prediction, or buy/sell/hold style suggestion, the answer must begin with a cautionary notice that the suggestion is based on past/current information and future market conditions may change outcomes.
...
* **FastAPI Supervisor Run API:** Supervisor workflows are now persisted backend runs. Creating a run requires selecting an existing company and one completed ingested document that belongs to that company. The API validates the company-document relationship, creates a run row, executes the supervisor workflow in the background, and stores per-agent status plus worker/supervisor JSON on the run row so the frontend can poll workflow history and run detail. During execution, the backend must route the company's normal/internal ticker into the fundamental worker and the company's Yahoo Finance ticker into the technical worker.
  The persisted worker JSON must now carry normalized visual-summary fields that the run-detail page can render directly without heuristically flattening arbitrary JSON into generic cards.
...
* **Run Detail Page:** The run detail view now uses a summary-first layout instead of one long stacked scroll. The top of the page must show compact company metadata, final rating, stance, confidence, and component score chips. Below that, the page must use tabs for `Overview`, `Fundamental`, `Technical`, and `News`.
  The `Overview` tab must show the final decision summary, top positives, top risks, watch items, and compact component-contribution bars. The `Fundamental` tab must lead with key business metrics and compact positive/risk/source blocks. The `Technical` tab must lead with the chart plus compact trend/setup/level metrics. The `News` tab must lead with sentiment and compact positive/negative/watch buckets with sources. The UI must stay minimal, use self-explanatory short labels, and avoid explanatory filler text.
  On newly created or still-running backend runs, tabs must not show mock evidence, placeholder cards, or seeded analysis copy; until a persisted worker result exists, the body stays blank apart from minimal waiting-state text. The technical tab must render the actual generated chart image used for analysis. The fundamental tab must render compiled source-page references for annual-report chunks. The news tab must render the source website list returned by the news worker.
...
* **Chat Pod:** A dedicated chat window is scoped to one completed stock run. Follow-up questions are answered by the supervisor against the existing worker outputs and stored workflow context through a FastAPI chat turn endpoint backed by the existing supervisor chat service. The backend chat path must reject out-of-scope prompts outside market, fundamentals, technicals, news, sector context, and supervisor-rating questions for the selected stock. Before supervisor completion, the chat pod must stay visually empty aside from a disabled-state message and must not display mock "result ready" content. On desktop run-detail layouts, the chat pod stays pinned on the right side while scrolling, its text composer stays anchored to the bottom edge of the pod, each submitted follow-up must show a visible in-flight assistant placeholder until the backend answer returns, and assistant answers must render as markdown rather than flat plain text.
...
* **Azure Container Apps Deployment:** Production deployment runs as one Azure Container Apps container that bundles the Next.js frontend, the FastAPI backend, and an internal nginx reverse proxy. nginx is the only public listener, forwarding `/api/*` and `/health` to FastAPI while routing all other paths to the Next.js server. The frontend must default to same-origin API requests when `NEXT_PUBLIC_API_BASE_URL` is unset so the deployed app works behind a single ACA hostname. HTTPS deployments must use secure auth cookies and set `FRONTEND_APP_URL`, `CORS_ORIGINS`, and `GOOGLE_OAUTH_REDIRECT_URI` to the final ACA hostname.
* **GitHub Actions CI/CD:** The repo must include a GitHub Actions workflow that runs backend tests plus frontend lint/build on every pull request and on pushes to `main`. Pushes to `main` must then deploy the single-container Azure Container Apps workload for this repo into resource group `feb26batch-e2e-project` by reusing the checked-in ACA deployment script. Deployment-time secrets must come from GitHub Actions secrets, while non-secret infrastructure settings should come from GitHub Actions variables. The deployment path must keep a stable `AUTH_SESSION_SECRET` so normal deployments do not invalidate existing user sessions.
