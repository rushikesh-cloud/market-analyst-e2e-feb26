          ...
          return (
            <div key={step.id} className="min-w-0">
              <div className="flex items-center gap-2">
                <span
                  className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border ${
                    done ? "border-emerald-200 bg-emerald-50 text-emerald-700" : active ? "border-blue-200 bg-blue-50 text-blue-700" : "border-line bg-slate-50 text-muted"
                  }`}
                >
              ...
              <div className="mt-2 h-1.5 rounded-full bg-slate-100">
                <div className={`h-full rounded-full ${done ? "w-full bg-emerald-500" : active ? "w-2/3 bg-blue-500" : "w-0 bg-slate-300"}`} />
              </div>
            </div>
          );
        })}
