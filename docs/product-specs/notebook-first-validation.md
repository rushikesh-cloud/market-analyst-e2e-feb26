...
| 04 | `04_fundamental_agent.ipynb` | Use RAG context to produce growth, debt, cash-flow, risk analysis, and a 1-100 fundamental rating. Fundamental RAG matching strips provider exchange suffixes such as `.NS` before comparing tickers. | `services.agents.fundamental` |
| 05 | `05_technical_agent.ipynb` | Pull price history with the exact supplied ticker, generate RSI/MACD/moving-average charts, send the chart to a multimodal Azure OpenAI deployment, and return a 1-100 technical rating from chart evidence. | `providers.market_data`, `services.charting`, `services.agents.technical` |
